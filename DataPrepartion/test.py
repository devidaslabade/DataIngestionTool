...
def launchSpark(src, schema, trgt):
    spark = SparkSession.builder.appName("DataIngestion").enableHiveSupport.getOrCreate()
    df = None
    if src['SrcType'] == "csv":
        df = spark.read.schema(schema).option("header", src['Header']).csv(src['SrcLocation'])
    elif src['SrcType'] == "json":
        df = spark.read.schema(schema).option("header", src['Header']).csv(src['SrcLocation'])
    elif src['SrcType'] == "parquet":
        df = spark.read.schema(schema).option("header", src['Header']).csv(src['SrcLocation'])
    elif src['SrcType'] == "orc":
        df = spark.read.schema(schema).option("header", src['Header']).csv(src['SrcLocation'])
    elif src['SrcType'] == "hivetable":
        df = spark.read.schema(schema).option("header", src['Header']).csv(src['SrcLocation'])
