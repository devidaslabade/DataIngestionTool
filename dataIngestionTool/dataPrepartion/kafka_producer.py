...
def publishKafka(spark, prcId,msg):
    ...
    try:
        ...
        print(prcKey)
        producer = KafkaProducer(bootstrap_servers='localhost:9092',value_serializer=lambda v: json.dumps(jsonString).encode('utf-8'))
        producer.send('data_ingestion', key=prcKey.encode(), value=b'jsonString')
